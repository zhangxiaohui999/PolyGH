...
# ———— step1 ————1.get_willUtg
# utg000005l_pilon_1_10000	hap	12	34	10000
# utg000005l_pilon_1_10000 10000 hap
def get_max_weight_list(addTwoFile_diff, willUtgFile_diff):
    willUtg_dict = {}
    with open(addTwoFile_diff, 'r') as fr, \
            open(willUtgFile_diff, 'w') as fw:
        lines = fr.readlines()
        for line in lines:
            splits = line.strip().split()
            utgName = splits[0]
            utgType = splits[1]
            utgSize = int(utgName.split('_')[-1]) - int(utgName.split('_')[-2]) + 1
            willUtg_dict[utgName] = {str(utgSize):utgType}
        # print(willUtg_dict)

        for utgName in willUtg_dict.keys():
            for utgSize in willUtg_dict[utgName].keys():
                fw.write(utgName+' '+utgSize+' '+willUtg_dict[utgName][utgSize]+'\n')


# ———— step2 ————2.hic_sumSize_48
list1 = [
...
def filterUtgRelation(filePath):
    willUtgDict = getWillUtg(filePath)  # 需要聚类的{utg:[size,type]}
    mySplit = '`?`'
    utgRelationDict = {}
    # utgUnikmerDict = {}
    with open('result.uni.log', 'r') as fr1:
        lines = fr1.readlines()
        for line in lines:
            splitArr = line.split(' ')
    ...
    # with open('utg_unik.log', 'r') as fr2:
    #     lines2 = fr2.readlines()
    #     for line in lines2:
    #         splits = line.strip().split()
    #         if splits[0] not in utgUnikmerDict.keys():
    #             utgUnikmerDict[splits[0]] = splits[1]
    
    selectClusterSize_1071 = {}
    with open('310.selectClusterSize_1071.txt','r') as fr3:
            lines = fr3.readlines()
            for line in lines:
                splits = line.strip().split()
    ...
    fw = open('hic_result.new.txt', 'w')  # <————————————————————————————————look look

    with open('result.uni.log', 'r') as fr:
        for line in fr:
            splits = line.strip().split()
                ...
                for suppId in utgList:
                    ...
                    if suppId[1:] in listid:
                        ...
                        if mySplit.join(lstTmp) in willUtgRelationDict.keys():
                            ...
                            # suppId是有@的
                            # if suppId in utgUnikmerDict.keys():
                            #     sizeSum *= int(utgUnikmerDict[suppId])
                            if suppId[1:][:16] in selectClusterSize_1071:
                                sizeSum += int(selectClusterSize_1071[suppId[1:][:16]])
                if sizeSum != 0:
                    fw.write(
                        willUtgId + '  in  list' + str(index+1) + '的uni_kmer数目 :  ' +
                        str(round(float((100000000 * relation) /
                                        sizeSum), 4)) + '\n')
                else:
                    fw.write(willUtgId + '  in  list' + str(index+1) + '的uni_kmer数目 :  ' +
                             '0' + '\n')
                index+=1
        fw.close()
...
def newSorted():
    lineData = []
    with open('hic_result.new.txt', 'r') as f2, open('hic_sorted.new.txt', 'w') as f3:
        lines2 = f2.readlines()
        # 对每48行排序
...
# ———— step3 ————3.normal_hic
def data_normal_48(filePath1, filePath2):
    lineData = []
    kmerData = []
    norm_data = []  # 归一化之后的list
    with open(filePath1, 'r') as fr, \
        ...
        for i in range(0, len(lines), 48):
            for line in lines:
                ...
                if len(kmerData) < 48:
                    kmerData.append(round(float(splits[-1]), 4))
                else:
                    k_min = min(kmerData)
                    k_max = max(kmerData)
                    dst = k_max - k_min
                    for data in kmerData:
                        if dst != 0:
                            norm_data.append((data - k_min) / (dst))
                        else:
                            norm_data.append(0)
                        # print(len(norm_data))
                    ...
                    i = 0
                    for line in lineData:
                        fw.write(line.strip().split(':')[0] + ':' + str(norm_data[i]) + '\n')
                        i += 1
                    norm_data = []
                    lineData = []
                    kmerData = []
...
if __name__ == '__main__':
    addTwoFile_diff = 'diff1.addTwo.350.txt'
    willUtgFile_diff = 'diff_willUtg_350.txt'
    get_max_weight_list(addTwoFile_diff, willUtgFile_diff)

    # 2
    # 步骤1得到的diff_willUtg_310.txt
    filterUtgRelation(willUtgFile_diff)
    # 这儿不用排序
    # newSorted()
    # sort  -k1,1 -k5,5nr hic_result.new.txt > hic_sorted.new.txt

    # 3
    filePath1 = 'hic_result.new.txt'
    filePath2 = 'hic_normal.new.txt'
    data_normal_48_new(filePath1, filePath2)