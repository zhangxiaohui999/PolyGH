...
def filterUtgRelation(utgWillFile, single_relationFile, resultFile):   
    ...
    willUtg_List = getWillUtg(utgWillFile)  # 需要聚类的{utg:[size,type]}  ['aa','bb','cc','dd'...]
    mySplit = '`?`'
    willUtgRelationDict = {}
    willutgDict = defaultdict(set)  # 存储和willUtg有关的list集合  {utg005:[utg001,utg002...]}

    with open(single_relationFile, 'r') as fr, \
            open(resultFile, 'w') as fw:
        for line in fr:
            splits = line.strip().split()
            willutgDict[splits[0]].add(splits[1]) # {'utg001': {'cc', 'bb', 'dd'...}}
            willUtgRelationDict[mySplit.join(splits[:2])] = splits[2]
        # print(willutgDict)
        for willUtgId in willUtg_List:
            utgList = willutgDict[willUtgId]  # 与willUtgId有关的utg集合 {'cc', 'bb', 'dd'...}
            ...
            for listid in listId_list:  # list1 ——> list48
                for suppId in utgList:  # 遍历与willUtgId有关的utg集合
                    lstTmp = [willUtgId, suppId]  # ['utg001', 'cc']
                    if suppId in listId[i]:
                        if mySplit.join(lstTmp) in willUtgRelationDict.keys():
                            relation = round(float(
                                willUtgRelationDict[mySplit.join(lstTmp)]), 16)

                            line = lstTmp[0] + ' ' + lstTmp[1] + ' ' + str(relation) + '\n'
                            relation_list.append(relation)
                            line_list.append(line)

                # 将两个列表打包成一个新的列表
                combined_list = list(zip(relation_list, line_list))
                # 根据第一个列表元素从大到小排序
                sorted_list = sorted(combined_list, key=lambda x: x[0], reverse=True)
                # 输出对应的第二个列表元素
                relation_list,line_list = [],[]

                listid_tmp = re.findall(r"\d+\.?\d*",listid)
                if len(sorted_list) != 0:
                    for item in sorted_list:
                        fw.write(willUtgId + '  in  list' + str(listid_tmp)[2:-2] + ' 的single关系:  ' + item[1])
                else:
                    fw.write(willUtgId + '  in  list' + str(listid_tmp)[2:-2] + ' 的single关系:  ' + '0' + '\n')
                i += 1

                # for item in sorted_list:
                #     fw.write(willUtgId + '  in  list' + str(index + 1) + ' 的single关系:  ' + item[1])
                # index += 1
                

if __name__ == '__main__':
    utgWillFile = '310.utgType.will.split50k.txt'
    single_relationFile = 's3_similarity_relation_0.01.new.txt'
    resultFile = 'result.single.tmp.txt'
    filterUtgRelation(utgWillFile, single_relationFile, resultFile)
